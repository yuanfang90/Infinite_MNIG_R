...
update_V<-function(a0,a1,a2,a3,a4,x,z,U,Uinv,d){
  ...
  for (g in 1:length(a0)){
    ...
    V <- A-V2-V3-V4-V5
    if (det(V)<0.00000001) {V0[[g]]<-solve(cov(x))}else{V0[[g]]<-solve(V,tol=(.Machine$double.neg.eps*1e-10))}
  }
  return(V0)
}
...
update_MuBeta<-function(a0,a1,a2,a3,a4,ISig,g,d){
  T_mu<-ISig[[g]]*a4[g]
  T_mubeta<-ISig[[g]]*a0[g]
  T_beta<-ISig[[g]]*a3[g]
  precision_mubeta<-rbind(cbind(T_mu,T_mubeta),cbind(T_mubeta,T_beta))
  for_cov<-solve(precision_mubeta,tol=(.Machine$double.neg.eps*1e-10))
...
sample_common<-function(up_p,V0,d){
  Mu <- list()
  Beta <- list()
  Gamma <- list()
  ISig <- list()
  ...
  for (g in 1:1){
    ...
    Mu_Beta<-update_MuBeta(a0=up_p$a0,a1=up_p$a1,a2=up_p$a2,a3=up_p$a3,a4=up_p$a4,ISig,g,d=d)
    Mu[[g]]<-Mu_Beta$Mu
    Beta[[g]]<-Mu_Beta$Beta
  }
...
lik <- function(mu,beta,gamma,Sigma_inv,x,d){
  lam = (d+1)/2
  mu_o<-as.matrix(mu)
...
dpMNIG <- function(data,true_lab,maxiter,dp.alpha,verbs=FALSE,plots=FALSE,outputs){
#################
#### DPMNIG #####
#################
x<-data
...
for(chain in 1:3){
  ...
  for (g in 1:G){
    com_par<-sample_common(up_p=up_p,V0=V0_co,d=d)
